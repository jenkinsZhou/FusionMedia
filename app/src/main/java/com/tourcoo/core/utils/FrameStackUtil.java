package com.tourcoo.core.utils;
 ...
 */
public class FrameStackUtil {
    private final String TAG = this.getClass().getSimpleName();
    private static Stack<Activity> mActivityStack;
    private static volatile FrameStackUtil sInstance;

    private FrameStackUtil() {
    }

    public static FrameStackUtil getInstance() {
        if (sInstance == null) {
            synchronized (FrameStackUtil.class) {
                if (sInstance == null) {
                    sInstance = new FrameStackUtil();
                }
            }
        }
        return sInstance;
     ...
     * @param activity
     */
    public FrameStackUtil push(Activity activity) {
        if (mActivityStack == null) {
            mActivityStack = new Stack();
        }
    ...
    public FrameStackUtil pop(Activity activity) {
     ...
     * @return
     */
    public FrameStackUtil pop(Activity activity, boolean isFinish) {
        if (activity != null) {
     ...
     * @param cls
     */
    public FrameStackUtil popAllExceptCurrent(Class cls) {
        while (true) {
            Activity activity = this.getCurrent();
     ...
     * 只留下栈顶一个Activity
     */
    public FrameStackUtil popAllExceptCurrent() {
        while (true) {
     ...
     * @return
     */
    public FrameStackUtil exit(boolean kill) {
        try {
            popAll();
            if (kill) {
