...
public abstract class BaseLoadingObserver<T> extends BaseObserver<T> {
     ...
     * @param activity
     */
    public BaseLoadingObserver(@Nullable Activity activity, IHttpRequestRefreshControl httpRequestControl, @StringRes int resId) {
        this(UiManager.getInstance().getLoadingDialog().createLoadingDialog(activity).setMessage(resId), httpRequestControl);
    }

    public BaseLoadingObserver(IHttpRequestRefreshControl httpRequestControl, @StringRes int resId) {
        this(StackUtil.getInstance().getCurrent(), httpRequestControl, resId);
    }

    public BaseLoadingObserver(@Nullable Activity activity, IHttpRequestRefreshControl httpRequestControl, CharSequence msg) {
        this(UiManager.getInstance().getLoadingDialog().createLoadingDialog(activity).setMessage(msg), httpRequestControl);
    }

    public BaseLoadingObserver(IHttpRequestRefreshControl httpRequestControl, CharSequence msg) {
    ...
    public BaseLoadingObserver(@Nullable Activity activity, IHttpRequestRefreshControl httpRequestControl) {
        this(UiManager.getInstance().getLoadingDialog().createLoadingDialog(activity), httpRequestControl);
    }

    public BaseLoadingObserver(IHttpRequestRefreshControl httpRequestControl) {
    ...
    public BaseLoadingObserver(LoadingDialogWrapper dialog, IHttpRequestRefreshControl httpRequestControl) {
