...
class FrameMultiUrl {
    ...
    private FrameUrlParser mUrlParser;
    private static volatile FrameMultiUrl sInstance;
    /**
     * retrofit 设置的BaseUrl
     */
    private String mBaseUrl;

    public static FrameMultiUrl getInstance() {
        if (sInstance == null) {
            synchronized (FrameMultiUrl.class) {
                if (sInstance == null) {
                    sInstance = new FrameMultiUrl();
                }
            }
        }
        return sInstance;
    ...
    private FrameMultiUrl() {
        //初始化解析器
        setUrlParser(new FrameUrlParser() {
            @Override
     ...
     * @return
     */
    public FrameMultiUrl with(OkHttpClient.Builder builder) {
    ...
    public Request processRequest(Request request) {
        ...
        if (null != httpUrl) {
            ...
            TourCooLogUtil.i(FrameMultiUrl.TAG, "Header 模式重定向Url:Base Url is { " + mBaseUrl + " }" + ";New Url is { " + newUrl + " }" + ";Old Url is { " + request.url() + " }");
            return newBuilder
                    .url(newUrl)
                    .build();
        }
        ...
        if (null != httpUrl) {
            TourCooLogUtil.i(FrameMultiUrl.TAG, "Method 模式重定向Url:Base Url is { " + mBaseUrl + " }" + ";New Url is { " + httpUrl + " }" + ";Old Url is { " + request.url() + " }");
            return newBuilder
                    .url(httpUrl)
                    .build();
        }
        ...
        if (httpUrlBase != null && !httpUrlBase.toString().equals(mBaseUrl)) {
            ...
            TourCooLogUtil.i(FrameMultiUrl.TAG, "重定向Url:Base Url is { " + httpUrlBase.toString() + " }" + ";New Url is { " + httpNew + " }" + ";Old Url is { " + request.url() + " }");
            return newBuilder
                    .url(httpNew)
                    .build();
        }
    ...
    private HttpUrl getMethodHttpUrl(Request request) {
        ...
        TourCooLogUtil.d(TAG, "Base Url is { " + mBaseUrl + " }" + ";Old Url is{" + url.newBuilder().toString() + "};Method is <<" + methodKey + ">>");
        //如果
     ...
     * @param enable
     */
    public FrameMultiUrl setIntercept(boolean enable) {
     ...
     * @return
     */
    public FrameMultiUrl setHeaderPriorityEnable(boolean enable) {
     ...
     * @param url
     */
    public FrameMultiUrl setGlobalBaseUrl(String url) {
    ...
    public FrameMultiUrl putHeaderBaseUrl(Map<String, String> map) {
     ...
     * @param urlValue
     */
    public FrameMultiUrl putHeaderBaseUrl(String urlKey, String urlValue) {
     ...
     * @param urlKey
     */
    public FrameMultiUrl removeHeaderBaseUrl(String urlKey) {
        synchronized (mHeaderBaseUrlMap) {
     ...
     * 清除所有BaseUrl
     */
    public FrameMultiUrl clearAllHeaderBaseUrl() {
        mHeaderBaseUrlMap.clear();
        return sInstance;
    }

    /**
     * 可自行实现 {@link FrameUrlParser} 动态切换 Url 解析策略
     *
     * @param parser
     */
    public FrameMultiUrl setUrlParser(FrameUrlParser parser) {
        this.mUrlParser = parser;
        return sInstance;
    }

    public FrameMultiUrl putBaseUrl(Map<String, String> map) {
        if (map != null && map.size() > 0) {
            for (Map.Entry<String, String> entry : map.entrySet()) {
     ...
     * @return
     */
    public FrameMultiUrl putBaseUrl(String urlKey, String urlValue) {
        synchronized (mBaseUrlMap) {
    ...
    public FrameMultiUrl removeBaseUrl(String urlKey) {
        synchronized (mBaseUrlMap) {
     ...
     * 清除所有BaseUrl
     */
    public FrameMultiUrl clearAllBaseUrl() {
        mBaseUrlMap.clear();
        return sInstance;
    }
    ...
    private HttpUrl checkUrl(String url) {
        ...
        if (null == parseUrl) {
            throw new NullPointerException(FrameConstant.EXCEPTION_EMPTY_URL);
        } else {
            return parseUrl;
        }
    }
